...
import com.epam.jf.bavtovich.homework.task7.AbstractIntArrayList;

public class Task7 extends com.epam.jf.bavtovich.homework.task7.AbstractIntArrayList {
    ...
    public int remove(int index) {
        isValidIndex(index);
        int[] cutValues = new int[values.length-1];
        int removedElem = values[index];
        System.arraycopy(values,0, cutValues, 0, index);
        System.arraycopy(values, index + 1, cutValues, index, values.length - index - 1);
        values = cutValues;
        return removedElem;
    }
    ...
    public AbstractIntArrayList subList(int fromInclusive, int toInclusive) {
        if (fromInclusive < toInclusive) {
            ...
            throw new IllegalArgumentException("fromInclusive index must be smaller then toInclusive index");
        }
    }
}
